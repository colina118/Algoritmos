#pragma once
#include <iostream>
#include "BinaryTree.h"


template <class T>
class Ordenamiento {
public:
	static void burbuja(T[], int, bool(*) (T, T));
	static void insercion(T[], int, bool(*) (T, T));
	static void seleccion(T[], int, bool(*) (T, T));
	static void quicksort(T[], int, int, bool(*) (T, T));
	static void mergesort(T[], int, int, bool(*) (T, T), int);
	static void merge(T[], int, int, int, bool(*) (T, T), int);
	static void cocktailSort(T[], int);
	static void bucketSort(T[], int);
	static void countingSort(T[], int, int);
	static void radixSort(T[], int, int);
	static void shellSort(T[], int);
	static void heapSort(T[], int);
	static void siftDown(T[], int, int);
	static void swap(int, int);

	static void binaryInsert(BNode<T> * node, int number);
	static void inOrder(BNode<T> *, T[], int &);
	static void treesort(T arr[], int);



	static bool asc(T, T);
	static bool desc(T, T);

};

template <class T>
bool Ordenamiento<T>::asc(T a, T b)
{
	return a > b;
}

template <class T>
bool Ordenamiento<T>::desc(T a, T b)
{
	return a < b;
}

template <class T>
void Ordenamiento<T>::burbuja(T v[], int n, bool compara(T, T))
{
	T temp;

	for (int i = 0; i < n; i++)
	{
		for (int j = n - 1; j > i; j--)
		{
			//if (v[j-1] > v[j])
			if (compara(v[j - 1], v[j]))
			{
				temp = v[j - 1];
				v[j - 1] = v[j];
				v[j] = temp;
			}
		}
	}
}

template <class T>
void Ordenamiento<T>::insercion(T v[], int n, bool compara(T, T))
{
	T index;

	for (int i = 1; i < n; i++)
	{
		index = v[i];

		int j = i - 1;

		//while (j >= 0 && v[j] > index)
		while (j >= 0 && compara(v[j], index))
		{
			v[j + 1] = v[j];
			j--;
		}

		v[j + 1] = index;
	}
}

template <class T>
void Ordenamiento<T>::seleccion(T v[], int n, bool compara(T, T))
{
	int minimo = 0;
	T temp;

	for (int i = 0; i < n - 1; i++)
	{
		minimo = i;
		for (int j = i + 1; j < n; j++)
		{
			//if (v[minimo] > v[j])
			if (compara(v[minimo], v[j]))
				minimo = j;
		}
		temp = v[minimo];
		v[minimo] = v[i];
		v[i] = temp;
	}
}

template <class T>
void Ordenamiento<T>::cocktailSort(T v[], int n)
{
	T temp;
	bool desordenado = true;
	while (desordenado)
	{
		desordenado = false;
		for (int i = 0; i < n - 1; i++)
		{
			if (v[i] > v[i + 1])
			{
				temp = v[i];
				v[i] = v[i + 1];
				v[i + 1] = temp;
				desordenado = true;
			}
		}
		if (!desordenado) break;
		desordenado = false;
		for (int j = n - 1; j > 0; j--)
		{
			if (v[j - 1] > v[j])
			{
				temp = v[j];
				v[j] = v[j - 1];
				v[j - 1] = temp;
				desordenado = true;
			}
		}
	}
}

template <class T>
void Ordenamiento<T>::bucketSort(T v[], int n)
{
	const int m = 101;
	int buckets[m];
	for (int i = 0; i < m; i++)
	{
		buckets[i] = 0;
	}
	for (int j = 0; j < n; j++)
	{
		++buckets[v[j]];
	}
	for (int i = 0, j = 0; j < m; j++)
	{
		for (int k = (buckets[j]); k > 0; k--)
		{
			v[i++] = j;
		}
	}
}

template <class T>
void Ordenamiento<T>::countingSort(T v[], int n, int k)
{
	int* contando = new int[k];
	int m = 0;
	for (int i = 0; i < k; i++)
	{
		contando[i] = 0;
	}
	for (int i = 0; i < n; i++)
	{
		contando[v[i]]++;
	}
	for (int i = 0; i < k; i++)
	{
		int cuenta = contando[i];
		for (int j = 0; j < cuenta; j++)
		{
			v[m] = i;
			m++;
		}

	}
}

template <class T>
void Ordenamiento<T>::mergesort(T v[], int l, int n, bool compara(T, T), int N)
{
	int m = (n + l) / 2;
	if (n > l)
	{
		mergesort(v, l, m, compara, N);
		mergesort(v, m + 1, n, compara, N);
		merge(v, l, m, n, compara, N);
	}
}

template <class T>
void Ordenamiento<T>::merge(T v[], int l, int m, int n, bool compara(T, T), int N)
{
	int i, j, k;
	T *aux = new T[N];   //Vector auxiliar

	for (i = m + 1; i>l; i--)
		aux[i - 1] = v[i - 1];

	for (j = m; j<n; j++)
		aux[n + m - j] = v[j + 1];

	for (k = l; k <= n; k++)       //Mezcla
		if (!compara(aux[i], aux[j])) // <
			v[k] = aux[i++];
		else
			v[k] = aux[j--];

	delete aux;
}

template<class T>
void Ordenamiento<T>::radixSort(T arr[], int n, int mod)
{
	T max = arr[0];
	for (int i = 1; i < n; i++)
	{
		if (arr[i] > max)
		{
			max = arr[i];
		}
	}

	for (int exp = 1; mod / exp > 0; exp *= 10)
	{
		countingSort(arr, n, mod);
	}
}

template<class T>
void Ordenamiento<T>::shellSort(T v[], int n)
{
	int k;
	for (int i = (n / 2); i > 0; i /= 2)
	{
		for (int j = i; j < n; j++)
		{
			int temp = v[j];
			for (k = j; k >= i && temp < v[k - i]; k -= i)
			{
				v[k] = v[k - i];
			}
			v[k] = temp;
		}
	}
}

template <class T>
void Ordenamiento<T>::heapSort(T a[], int n) {

	for (int k = (n / 2); k >= 0; k--)
	{
		siftDown(a, k, n);
	}

	while (n - 1 > 0)
	{
		/* swap the root(maximum value) of the heap
		with the last element of the heap */
		swap(a[n - 1], a[0]);

		siftDown(a, 0, n - 1);
		/* N-- : decrease the size of the heap by one
		so that the previous max value will
		stay in its proper placement */
		n--;
	}
}

template <class T>
void Ordenamiento<T>::siftDown(T *a, int k, int n)
{
	while (k * 2 + 1 < n)
	{

		int child = 2 * k + 1;


		if ((child + 1 < n) && (a[child] < a[child + 1])) child++;

		if (a[k] < a[child])
		{

			swap(a[child], a[k]);

			k = child;
		}
		else
			return;
	}
}

template <class T>
void Ordenamiento<T>::swap(int m, int n)
{
	T tmp;
	tmp = m;
	m = n;
	n = tmp;
}

template <class T>
void Ordenamiento<T>::quicksort(T v[], int primero, int ultimo, bool compara(T, T))
{
	int izquierdo = primero;
	int derecho = ultimo;
	T temp;

	//Se selecciona pivote
	T pivote = v[primero];

	if (primero < ultimo) // Paso base
	{
		// particion
		while (izquierdo < derecho)
		{
			while ((izquierdo < derecho) && !compara(v[izquierdo], pivote)) izquierdo++; // <=
			while (compara(v[derecho], pivote)) derecho--; // >

			if (izquierdo < derecho) // se intercambian los contenidos
			{
				temp = v[derecho];
				v[derecho] = v[izquierdo];
				v[izquierdo] = temp;
			}
		}

		// Se intercambia el pivote con el elemento de la posición derecha
		temp = v[derecho];
		v[derecho] = v[primero];
		v[primero] = temp;

		// Paso recursivo
		quicksort(v, primero, derecho - 1, compara);
		quicksort(v, izquierdo, ultimo, compara);
	}
}

template <class T>
void Ordenamiento<T>::binaryInsert(BNode<T> * node, int number)
{
	if (node->getInfo() > number)
	{
		if (!node->getLeft())
		{
			BNode<T> * val = new BNode<T>(number);
			node->setLeft(val);
		}
		else
		{
			binaryInsert(node->getLeft(), number);
		}
	}
	else
	{
		if (!node->getRight())
		{
			BNode<T> * val = new BNode<T>(number);
			node->setRight(val);
		}
		else
		{
			binaryInsert(node->getRight(), number);
		}
	}

}

template <class T>
void Ordenamiento<T>::inOrder(BNode<T> * node, T *arr, int &pos)
{
	if (node)
	{
		inOrder(node->getLeft(), arr, pos);
		*(arr + pos) = node->getInfo();
		pos++;
		inOrder(node->getRight(), arr, pos);
	}
}

template <class T>
void Ordenamiento<T>::treesort(T arr[], int n)
{
	BNode<T> * root = new BNode<T>(arr[0]);
	BinaryTree<T> * tree = new BinaryTree<T>;
	tree->setRoot(root);
	for (int i = 1; i < n; ++i)
	{
		binaryInsert(root, arr[i]);
	}
	int pos = 0;
	inOrder(root, arr, pos);
	delete tree;

}